...
export const exchangeSwarmAddresses = async (
  ...
  if (network) {
    const isTrusted = (peer: PeersResult) =>
      network.isTrusted(new IPFSAddress({ address: peer.peer.toString() }));
  ...
  const filteredAddresses = trustedAddresses
    .filter(
      (x) =>
        x.peer.toString() !== peerReciever &&
        (localNetwork || !isLocalhostAddress(x.peer.toString()))
    )
    .map((x) => {
      return new PeerInfo({
        id: x.peer.toString(),
