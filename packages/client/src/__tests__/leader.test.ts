...
describe(`leaders`, function () {
    ...
    it("will use trusted network for filtering", async () => {
        ...
        await waitFor(
            () =>
                client1.getReplicators(program.address!.toString())?.length ===
                1
        );
        await waitFor(
            () =>
                client2.getReplicators(program.address!.toString())?.length ===
                1
        );
    ...
    it("leader are selected from 1 replicating peer", async () => {
        ...
        const isLeaderA = await client1.isLeader(
            db1.address!.toString(),
            slot,
            2
        );
        const isLeaderB = await client2.isLeader(
            db1.address!.toString(),
            slot,
            2
        );
    ...
    it("leader are selected from 2 replicating peers", async () => {
        ...
        const isLeaderC = await client3.isLeader(
            db1.address!.toString(),
            slot,
            3
        );
