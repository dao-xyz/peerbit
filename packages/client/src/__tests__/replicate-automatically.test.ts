  ...
  describe(`orbit-db - Automatic Replication (${API})`, function () {
    ...
    it("starts replicating the database when peers connect", async () => {
      ...
      const db3 = await orbitdb1.open(
        new KeyValueStore<string>({ id: "replicate-automatically-tests-kv" }),
        {
          replicationTopic,
      ...
      const db2 = await orbitdb2.open<EventStore<string>>(
        await EventStore.load<EventStore<string>>(orbitdb2._ipfs, db1.address!),
        {
          replicationTopic,
          onReplicationComplete: (_) => {
      ...
      const _db4 = await orbitdb2.open<KeyValueStore<string>>(
        await KeyValueStore.load<KeyValueStore<string>>(
          orbitdb2._ipfs,
          db3.address!
        ),
        {
