...
export class Peerbit {
  ...
  static async create(ipfs: IPFS, options: CreateInstanceOptions = {}) {
    ...
    const cache =
      options.cache ||
      new Cache(
        await storage.createStore(path.join(directory, id.toString(), "/cache"))
      );
    ...
    */
          ...
          this._ipfs.swarm.connect(
            multiaddr(
              info.address.toString() +
                (info.address.indexOf(suffix) === -1 ? suffix : "")
            )
          );
        });
        ...
        if (msg.condition instanceof RequestKeysByAddress) {
          ...
          const canExchangeKey: KeyAccessCondition = (key) =>
            key.group === (msg.condition as RequestKeysByAddress<any>).address;
  ...
  async _onPeerConnected(replicationTopic: string, peer: string) {
    ...
    try {
      ...
      if (programs) {
        ...
        const initializeAsReplicator = async () => {
          await exchangeSwarmAddresses(
            (data) => this._ipfs.pubsub.publish(replicationTopic, data),
            this.identity,
            peer,
      ...
      logger.error(
        "Unexpected error in _onPeerConnected callback: " + error.toString()
      );
      throw error;
    }
  }
  ...
  async replicationReorganization(modifiedChannel: DirectChannel) {
    ...
    if (!connections) {
      logger.error(
        "Missing direct connection to: " + modifiedChannel.recieverId.toString()
      );
      return;
    }
  ...
  async getChannel(
            ...
            onNewPeerCallback: (channel) => {
              ...
              if (!this._directConnections.has(channel.recieverId.toString())) {
                this._directConnections.set(
                  ...
                  new SharedChannel(channel, new Set([fromTopic]))
                );
              } else {
                this._directConnections
                  .get(channel.recieverId.toString())
    ...
    try {
      ...
      const connected = await channel.connect({
        isClosed: () => cancel || this._disconnected || this._disconnecting,
      });
      if (!connected) {
  ...
  async _onClose(program: Program, db: Store<any>, replicationTopic: string) {
    ...
    const dir =
      db && db._options.directory ? db._options.directory : this.directory;
  ...
  async _onProgamClose(program: Program, replicationTopic: string) {
    ...
    const otherStoresUsingSameReplicationTopic =
      this.programs.get(replicationTopic);
  ...
  async findLeaders(
    ...
    const isTrusted = (peer: string | PeerId) =>
      network
        ? network.isTrusted(new IPFSAddress({ address: peer.toString() }))
        : true;
    const peers = await Promise.all(allPeers.map(isTrusted)).then((results) =>
  ...
  async open<S extends Program>(
    ...
    const fn = async (): Promise<ProgramWithMetadata> => {
      ...
      const definedReplicationTopic: string = (options.replicationTopic ||
        programAddress)!;
      ...
      if (
        ...
        if (!program.owner) {
          ...
          let ownerProgram: AbstractProgram | undefined = this.programs
            .get(definedReplicationTopic)
          ...
          this._waitForKeysTimeout
            ? (key) =>
                this.requestAndWaitForKeys(
                  definedReplicationTopic,
                  programAddress,
                  new RequestKeysByKey<Ed25519Keypair | X25519Keypair>({
                    key,
                  })
                )
            : undefined
