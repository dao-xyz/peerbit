...
describe('Identity Provider', function () {
  ...
  describe('Passing in custom keystore', () => {
    ...
    it('has the correct id', async () => {
      ...
      const key = await keystore.getKeyByPath<KeyWithMeta<Ed25519Keypair>>(id)
      const externalId = key.publicKey;
      assert.deepStrictEqual(identity.id, new Uint8Array(externalId.getBuffer()))
    })
    ...
    it('has the correct public key', async () => {
      ...
      const signingKey = await keystore.getKeyByPath<KeyWithMeta<Ed25519Keypair>>(externalId)
    ...
    it('has a signature for the publicKey', async () => {
      const key = await keystore.getKeyByPath<KeyWithMeta<Ed25519Keypair>>(id)
      const externalId = key.publicKey;
      const signingKey = await keystore.getKeyByPath<KeyWithMeta<Ed25519Keypair>>(externalId.toString('base64'))
      const idSignature = await Keystore.sign(new Uint8Array(externalId.getBuffer()), signingKey)
      const externalKey = await keystore.getKeyByPath<KeyWithMeta<Ed25519Keypair>>(id)
  ...
  describe('create an identity with saved keys', () => {
    ...
    it('has the correct id', async () => {
      const key = await savedKeysKeystore.getKeyByPath<KeyWithMeta<Ed25519Keypair>>(id)
    ...
    it('has the correct signatures', async () => {
      const internalSigningKey = await savedKeysKeystore.getKeyByPath<KeyWithMeta<Ed25519Keypair>>(identity.id)
      const externalSigningKey = await savedKeysKeystore.getKeyByPath<KeyWithMeta<Ed25519Keypair>>(id)
