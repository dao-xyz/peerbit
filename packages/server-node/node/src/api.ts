...
import { Program, Address, Replicator } from "@dao-xyz/peerbit-program";
						...
						switch (req.method) {
							case "GET":
								import("url").then((parse) => {
									const replicateParam = parse.parse(req.url!, true).query[
										"replicate"
									];
									...
									if (replicate) {
											...
											res.write(
												JSON.stringify(
													[...client.programs.entries()]
														.filter(
															(x) => x[1].program.role instanceof Replicator
														)
														.map((x) => x[0])
												)
											);
										...
										res.write(
											JSON.stringify([
												...libp2p.services.pubsub.subscriptions.keys(),
											])
										);
										res.end();
									}
		...
		topics: {
			get: async (replicate: boolean): Promise<string[]> => {
				const result = throwIfNot200(
					await axios.get(endpoint + TOPICS_PATH + "?replicate=" + replicate, {
						validateStatus,
						headers: await getHeaders(),
					})
				);
