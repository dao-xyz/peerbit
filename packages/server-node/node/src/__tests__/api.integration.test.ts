...
describe("libp2p only", () => {
	...
	beforeEach(async () => {
		session.peers[0].directsub.subscribe("1", {
			data: new Uint8Array([1]),
		});
		session.peers[0].directsub.subscribe("2", {
			data: new Uint8Array([2]),
		});
		session.peers[0].directsub.subscribe("3", {
...
describe("server", () => {
	...
	describe("ipfs", () => {
		it("id", async () => {
			...
			expect(await c.peer.id.get()).toEqual(peer.libp2p.peerId.toString());
		});
		it("addresses", async () => {
			const c = await client();
