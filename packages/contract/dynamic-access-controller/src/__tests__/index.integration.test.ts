...
@variant([0, 251])
class TestStore<T extends BinaryPayload> extends Contract {

    @field({ type: BinaryDocumentStore })
    store: BinaryDocumentStore<T>
...
describe('index', () => {
    ...
    it('can write from trust web', async () => {
        ...
        await (l0b.accessController).acl.trustedNetwork.trustGraph.sync((l0a.accessController).acl.trustedNetwork.trustGraph.oplog.heads);
        ...
        await l0a.store.sync(l0b.store.oplog.heads);
        await l0b.store.sync(l0a.store.oplog.heads);
    ...
    describe('conditions', () => {
        it('publickey', async () => {
            ...
            await (l0b.accessController).acl.access.sync((l0a.accessController).acl.access.oplog.heads);
        ...
        it('through trust chain', async () => {
            ...
            await (l0b.accessController).acl.access.sync((l0a.accessController).acl.access.oplog.heads);
            await (l0c.accessController).acl.access.sync((l0a.accessController).acl.access.oplog.heads);
            ...
            await (l0c.accessController).acl.identityGraphController.relationGraph.sync((l0b.accessController).acl.identityGraphController.relationGraph.oplog.heads);
    ...
    it('can query', async () => {
        ...
        (l0b.accessController).acl.access.sync((l0a.accessController).acl.access.oplog.heads)
