  ...
  describe("Log - Load", function () {
    ...
    describe("fromEntry", () => {
      ...
      it("retrieves full log from an entry hash 3", async () => {
        ...
        expect(c.values.map((e) => e.payload.getValue())).toContainAllValues(
          tmp
        );
      ...
      it("retrieves randomly joined log deterministically", async () => {
        ...
        const logB = new Log(
          ipfs,
          { ...signKey2.keypair, sign: (data) => signKey2.keypair.sign(data) },
          { logId: "X" }
        );
        const log3 = new Log(
          ipfs,
          { ...signKey3.keypair, sign: (data) => signKey3.keypair.sign(data) },
          { logId: "X" }
        );
        const log = new Log(
          ipfs,
          { ...signKey4.keypair, sign: (data) => signKey4.keypair.sign(data) },
          { logId: "X" }
        );
