...
Object.keys(testAPIs).forEach((IPFS) => {
  describe('Log - CRDT', function () {
    ...
    beforeAll(async () => {
      ...
      signKey = await keystore.createKey(await Ed25519Keypair.create(), { id: new Uint8Array([0]), overwrite: true });
      signKey2 = await await keystore.createKey(await Ed25519Keypair.create(), { id: new Uint8Array([2]), overwrite: true });
      signKey3 = await await keystore.createKey(await Ed25519Keypair.create(), { id: new Uint8Array([3]), overwrite: true });
