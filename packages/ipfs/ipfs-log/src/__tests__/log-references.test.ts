  ...
  describe('Log - References', function () {
    ...
    describe('References', () => {
      /*   it('Power of 2 references', async () => {
          ...
          const log1 = new Log(ipfs, signKey.publicKey, (data) => Keystore.sign(data, signKey), { gid:  'A' })
          const log2 = new Log(ipfs, signKey.publicKey, (data) => Keystore.sign(data, signKey), { gid:  'B' })
          const log3 = new Log(ipfs, signKey.publicKey, (data) => Keystore.sign(data, signKey), { gid:  'C' })
          const log4 = new Log(ipfs, signKey.publicKey, (data) => Keystore.sign(data, signKey), { gid:  'D' })
          ...
          for (const input of inputs) {
            const test = async (amount: number, referenceCount: number, refLength: number) => {
              const log1 = new Log(ipfs, signKey.publicKey, (data) => Keystore.sign(data, signKey), { gid:  'A' })
              for (let i = 0; i < amount; i++) {
                await log1.append((i + 1).toString(), { nexts: log1.getPow2Refs(referenceCount) })
              }
