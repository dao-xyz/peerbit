...
describe("ipfs-log - Replication", function () {
    ...
    describe("replicates logs deterministically", function () {
        ...
        afterEach(async () => {
            await session.peers[0].ipfs.pubsub.unsubscribe(
                channel,
                handleMessage
            );
            await session.peers[1].ipfs.pubsub.unsubscribe(
                channel,
                handleMessage2
            );
        });
        ...
        it("replicates logs", async () => {
            ...
            for (let i = 1; i <= amount; i++) {
                ...
                await session.peers[0].ipfs.pubsub.publish(
                    channel,
                    Buffer.from(hash1)
                );
                await session.peers[1].ipfs.pubsub.publish(
                    channel,
                    Buffer.from(hash2)
                );
            }
