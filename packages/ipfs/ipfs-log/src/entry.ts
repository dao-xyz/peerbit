...
export const maxClockTimeReducer = <T>(res: bigint, acc: Entry<T>): bigint =>
  max(res, acc.clock.time);
...
export class Payload<T> {
  ...
  equals(other: Payload<T>): boolean {
    return (
      Buffer.compare(Buffer.from(this.data), Buffer.from(other.data)) === 0
    );
  }
...
export class Entry<T>
  implements EntryEncryptionTemplate<Clock, Payload<T>, SignatureWithKey>
{
  @field({ type: "string" })
  gid: string; // graph id
  ...
  init(
    props:
      | { encryption?: PublicKeyEncryptionResolver; encoding: Encoding<T> }
      | Entry<T>
  ): Entry<T> {
    const encryption =
  ...
  static createDataToSign(
    gid: string,
    payload: MaybeEncrypted<Payload<any>>,
    clock: MaybeEncrypted<Clock>,
    next: string[],
    fork: string[],
    state: number,
  ...
  equals(other: Entry<T>) {
    return (
      this.gid === other.gid &&
      this.maxChainLength === other.maxChainLength &&
      this._reserved === other._reserved &&
      this._state === other._state &&
      this._clock.equals(other._clock) &&
      this._signature.equals(other._signature) &&
      arraysEqual(this.next, other.next) &&
      arraysEqual(this._forks, other._forks) &&
  ...
  static async create<T>(properties: {
    ...
    if (!clockValue) {
      const newTime =
        nexts?.length > 0 ? nexts.reduce(maxClockTimeReducer, 0n) + 1n : 0n;
      if (
        properties.encryption?.reciever.signature &&
        properties.encryption?.reciever.clock
      ) {
        throw new Error(
      ...
      clockValue = new Clock(
        new Uint8Array(serialize(properties.identity.publicKey)),
        newTime
      );
    } else {
      const cv = clockValue;
    ...
    const clock = await maybeEncrypt(
      clockValue,
      properties.encryption?.reciever.clock
    );
      ...
      nexts.forEach((n) => {
        ...
        if (
          ...
          // replace gid if next is from alonger chain, or from a later time, or same time but "smaller" gid
            ...
            maxChainLength < n.maxChainLength ||
            maxClock < n.clock.time ||
            (maxClock == n.clock.time && n.gid < gid)
          ) {
            gid = n.gid;
          }
        }
    ...
    const signature = await properties.identity.sign(
      Entry.createDataToSign(gid, payload, clock, next, forks, state, reserved)
    );
  ...
  static async toMultihash<T>(ipfs: IPFS, entry: Entry<T>, pin = false) {
    ...
    return io.write(ipfs, "dag-cbor", entry.serialize(), {
  ...
  static toEntry<T>(entry: EntrySerialized<T>): Entry<T> {
    ...
    if (entry instanceof Entry) {
      ...
      clock = deserialize<MaybeEncrypted<Clock>>(
        Buffer.from(entry.clock),
        MaybeEncrypted
      );
      payload = deserialize<MaybeEncrypted<Payload<T>>>(
        Buffer.from(entry.payload),
        MaybeEncrypted
      );
      signature = deserialize<MaybeEncrypted<SignatureWithKey>>(
  ...
  static async fromMultihash<T>(ipfs: IPFS, hash: string) {
    ...
    const e: EntrySerialized<T> = await io.read(ipfs, hash, {
      links: Entry.IPLD_LINKS,
    });
