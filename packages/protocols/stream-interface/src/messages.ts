...
export class DataMessage extends Message {
	...
	private _data: MaybeEncrypted<Uint8Array>;

	constructor(properties: {
		header?: MessageHeader;
		to?: string[];
		data: MaybeEncrypted<Uint8Array>;
	...
	get data(): MaybeEncrypted<Uint8Array> {
		return this._data;
	}
	...
	async createPrefix(): Promise<Uint8Array> {
		...
		const hashBytes = await sha256(serialize(this.data));
