			...
			it("concurrently", async () => {
				...
				const fn = async (i: number) => {
					...
					await metrics[i % 3].stream.publish(d, {
						to: [
							metrics[(i + 1) % session.peers.length].stream.publicKey,
							metrics[(i + 2) % session.peers.length].stream.publicKey,
						],
						strict: true,
						topics: [TOPIC],
					});
		...
		describe("emitSelf", () => {
			...
			it("publish", async () => {
				...
				await session.peers[0].services.pubsub.publish(
					...
					{ to: [session.peers[1].services.pubsub.publicKey] }
				);
