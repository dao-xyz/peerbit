...
>
	...
	async createMessage(
		...
		options?:
			...
			| { encrypt?: false; to: Ed25519PublicKey[], keypair?: X25519Keypair, }
			| {
				encrypt: true; to: (string | PublicSignKey | PeerId)[], keypair?: X25519Keypair, recievers?: X25519PublicKey[]
			}
	) {
		...
		const dataArray = data instanceof Uint8ArrayList ? data.subarray() : data;

		const message = new DataMessage({
			data:
				options?.encrypt === true
					? await new DecryptedThing<Uint8Array>({ data: dataArray }).encrypt(
						options.keypair || await X25519Keypair.create(),
						...(options.recievers ? options.recievers : options.to as Ed25519PublicKey[])
					)
					: new DecryptedThing({ data: dataArray }),
			to: toHashes,
		});
