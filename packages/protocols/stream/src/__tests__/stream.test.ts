...
import { DecryptedThing, PublicSignKey } from "@peerbit/crypto";
	...
	describe("publish", () => {
		...
		describe("shortest path", () => {
			...
			it("1->unknown", async () => {
				...
				expect(
					new Uint8Array(metrics[1].recieved[0].data.decrypted.data)
				).toEqual(data);
				await waitFor(() => metrics[2].recieved.length === 1);
				expect(
					new Uint8Array(metrics[2].recieved[0].data.decrypted.data)
				).toEqual(data);
			...
			it("1->2", async () => {
				...
				expect(new Uint8Array(recievedMessage.data.decrypted.data)).toEqual(
					data
				);
				await delay(1000); // wait some more time to make sure we dont get more messages
				expect(metrics[1].recieved).toHaveLength(1);
			...
			describe("basic", () => {
				...
				it("will not publish to from when explicitly providing to", async () => {
					const msg = new DataMessage({
						data: new DecryptedThing({ data: new Uint8Array([0]) }),
					});
	...
	describe("join/leave", () => {
		...
		describe("direct connections", () => {
			beforeEach(async () => {
				...
				); // Second arg is due to https://github.com/protocols/js-libp2p/issues/1690
				metrics = [];
