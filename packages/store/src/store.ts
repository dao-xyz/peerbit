...
export class Store<T> extends SystemBinaryPayload implements Initiable<T> {
  ...
  updateStateFromLogs = async (logs: Log<T>[]) => {
    if (this._oplog && logs.length > 0) {
      ...
      logger.debug(
        `Saved heads ${heads.length} [${heads.map((e) => e.hash).join(", ")}]`
      );
  ...
  async load(amount?: number, opts: { fetchEntryTimeout?: number } = {}) {
    ...
    const localHeads: Entry<any>[] =
      (await this._cache.getBinary(this.localHeadsPath, HeadsCache))?.heads ||
      [];
    const remoteHeads: Entry<any>[] =
      (await this._cache.getBinary(this.remoteHeadsPath, HeadsCache))?.heads ||
      [];
