    ...
    encryption = {
      ...
      getAnyKeypair: async (publicKeys: X25519PublicKey[]) => {
        for (let i = 0; i < publicKeys.length; i++) {
          if (
            publicKeys[i].equals(
              await X25519PublicKey.from(senderKey.keypair.publicKey)
            )
          ) {
            return {
              index: i,
          ...
          if (
            publicKeys[i].equals(
              await X25519PublicKey.from(recieverKey.keypair.publicKey)
            )
          ) {
            return {
              index: i,
  ...
  it("encrypted entry is appended known key", async () => {
    ...
    const onWrite = async (store: Store<any>, entry: Entry<any>) => {
      try {
        ...
        await waitForAsync(
          async () =>
            (await store._cache.getBinary(store.localHeadsPath, HeadsCache)) !==
            undefined
        );
