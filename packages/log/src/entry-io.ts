	...
	static async fetchAll<T>(
		...
		const addToLoadingQueue = (e: Entry<T> | string, ts: bigint) => {
			...
			if (
				...
				loadingQueue.sort((a, b) => (a.ts > b.ts ? -1 : a.ts === b.ts ? 0 : 1)); // ascending
		...
		const addToResults = async (entry: Entry<T>) => {
			if (!cache.has(entry.hash) && shouldFetch(entry.hash)) {
				...
				minClock =
					result.length > 0
						? Timestamp.bigger(
								(await result[result.length - 1].metadata).clock.timestamp,
								minClock
						  )
						: maxClock;
				...
				if (
					(options.length < 0 || result.length < options.length || isLater) &&
				...
				if (options.length < 0) {
					...
					nextSorted.forEach((e) => addToLoadingQueue(e, ts.wallTime));
				} else {
		...
		const fetchEntry = async (hash: string) => {
			...
			return new Promise(async (resolve, reject) => {
				...
				try {
					// Load the entry
					const entry = await Entry.fromMultihash<T>(store, hash, options);
