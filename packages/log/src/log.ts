...
export class Log<T> {
	...
	async open(store: Blocks, identity: Identity, options: LogOptions<T> = {}) {
		...
		this._storage = store;
		this._indexer = indexer || new HashmapIndices();
	...
	async append(
		...
		const nexts: Entry<any>[] | ShallowEntry[] = options.meta?.next || await this.entryIndex.getHeads().all();
		...
		await this.entryIndex.put(entry, { isHead: true, toMultiHash: false });
	...
	async join(
		entriesOrLog: (string | Entry<T> | EntryWithRefs<T>)[] | Log<T> | ResultsIterator<Entry<any>>,
		options?: {
	...
	private async joinRecursively(entry: Entry<T>, options: {
		...
		const headsWithGid = await this.entryIndex.getHeads(entry.gid, false).all();
		if (headsWithGid) {
		...
		await this._entryIndex.put(entry, { isHead: options.isHead, toMultiHash: true });
	...
	/* async join(
		...
		let entries: (string | Entry<T> | EntryWithRefs<T>)[]
		...
		if (entriesOrLog instanceof Log) {
			...
			entries = entriesOrLog
			console.log("JOIN", entries.map(x => x instanceof Entry ? x.hash + "/" + x.meta.clock.timestamp.wallTime : "_"))
		...
		for (const hash of stack) {
			if (visited.has(hash) || await this.has(hash)) {
				continue;
			}
			visited.add(hash);
			...
			if (
				...
				for (const next of nexts) {
					...
					console.log("ADD NEXT", !visited.has(next), entry.hash + "/" + entry.meta.clock.timestamp.wallTime, next);
				...
				if (isRoot) {
					console.log("IS ROOT!", entry.hash + "/" + entry.meta.clock.timestamp.wallTime)
					entriesBottomUp.push(entry);
				}
			} else {
				console.log("XXX!", entry.hash + "/" + entry.meta.clock.timestamp.wallTime)
				entriesBottomUp.push(entry);
			}
		}

		console.log("---> ", entriesBottomUp.map(x => x.hash + "/" + x.meta.clock.timestamp.wallTime))

		while (entriesBottomUp.length > 0) {
			const e = entriesBottomUp.shift()!;
			await this._joining.get(e.hash);
			for (const next of e.meta.next) {
				await this._joining.get(next);
			}
	...
	private async joinEntry(
		...
		console.log("JOIN ENTRY", e.hash, e.meta.next)

		// check that all nexts are in index
		for (const next of e.meta.next) {
			if (!await this.has(next)) {
				throw new Error("Unexpected");
			}
		...
		const headsWithGid = await this.entryIndex.getHeads(e.gid, false).all();
		if (headsWithGid) {
			for (const v of headsWithGid) {
				if (v.meta.type === EntryType.CUT && v.meta.next.includes(e.hash)) { // TODO second argument should be a time compare instead? what about next nexts?
					return; // already deleted
				}
			}
		}
		...
		if (!await this.has(e.hash)) {
			...
			const isHead = !nextRefs.has(e.hash);
			console.log("PUT ENTRY", e.hash + "/" + e.meta.clock.timestamp.wallTime, isHead, nextRefs.has(e.hash));
			await this._entryIndex.put(e, { isHead, toMultiHash: true });
		...
		if (forward) {
			for (const en of forward) {
				console.log("REC JOIN", en.hash + "/" + en.meta.clock.timestamp.wallTime);
				stack.push(en);
			}
		}
	} */
	...
	async load(
		...
		if (this._loadedOnce && !opts.reload) {
			return;
		}
