...
export class Trim<T> {
	...
	async deleteFromCache(hash: string) {
		if (this._canTrimCacheLastNode?.hash === hash) {
			this._canTrimCacheLastNode = await this._log.index.getBefore(this._canTrimCacheLastNode, false)
		}
	}
	...
	private async trimTask(
		...
		} /* else if (option.type == "bytelength") { TODO
			// prune to max sum payload sizes in bytes
			const byteLengthFrom = option.from ?? option.to;

			if (valueIterator.byteLength < byteLengthFrom) {
				return [];
			}
			done = () => valueIterator.byteLength <= option.to;
