...
describe("from", function () {
	...
	describe("fromEntryHash", () => {
		it("creates a log from an entry hash", async () => {
			...
			expect(
				(await log1.getHeads().all()).map((x) => x.meta.gid),
			).to.have.members((await data.getHeads().all()).map((x) => x.meta.gid));
		...
		it("respects timeout parameter", async () => {
			...
			try {
				...
				expect(et - st).greaterThan(timeout);
				expect(et - st).lessThan(timeout * 100); // some upper bound
			}
		});
	});
	...
	describe("fromEntry", () => {
		...
		it("keeps the original heads", async () => {
			...
			expect((await log2.getHeads())[0].gid).equal(
				(await data.getHeads())[0].gid
			);
			expect(log2.length).equal(4);
			expect((await log2.toArray())[0].payload.getValue()).equal("entryC0");
			expect((await log2.toArray())[1].payload.getValue()).equal("entryA8");
			expect((await log2.toArray())[2].payload.getValue()).equal("entryA9");
			expect((await log2.toArray())[3].payload.getValue()).equal("entryA10");
			...
			expect((await log3.getHeads())[0].gid).equal(
				(await data.getHeads())[0].gid
			);
			expect(log3.length).equal(7);
			expect((await log3.toArray())[0].payload.getValue()).equal("entryB5");
			expect((await log3.toArray())[1].payload.getValue()).equal("entryA6");
			expect((await log3.toArray())[2].payload.getValue()).equal("entryC0");
			expect((await log3.toArray())[3].payload.getValue()).equal("entryA7");
			expect((await log3.toArray())[4].payload.getValue()).equal("entryA8");
			expect((await log3.toArray())[5].payload.getValue()).equal("entryA9");
			expect((await log3.toArray())[6].payload.getValue()).equal("entryA10");
		}); 
	 ...
	 expect(g.toString()).equal(bigLogString) */
		...
		it("retrieves partially joined log deterministically - single next pointer", async () => {
			...
			expect((await res.toArray()).map((e) => e.payload.getValue())).equal(
				first11
			);
