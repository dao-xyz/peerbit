...
const checkHashes = async (
	...
	if (hashes.length > 0) {
		for (let i = 0; i < hashes.length; i++) {
			...
			expect(headCache?.heads).to.have.members(hashes[i]);
			if (i === hashes.length - 1) {
				expect(headCache?.last).equal(undefined);
			} else {
				expect(headCache?.last).to.exist;
...
describe(`head-cache`, function () {
	...
	it("closes and loads", async () => {
		...
		await log.append(data).then((entry) => {
			expect(entry.entry).to.be.instanceOf(Entry);
		});
		...
		await log.load();
		expect(log.closed).to.be.false;
		expect(log.values.length).equal(1);

		await log.close();
		expect(await level.status()).equal("closed");
	});
	...
	it("loads when missing cache", async () => {
		...
		await init(cache);
		await log.load();
		expect(log.values.length).equal(0);
		await log.close();
	});
	...
	it("will respect deleted heads", async () => {
		...
		).to.have.members([e1.hash, e2.hash, e3.hash]);

		// Remove e1
		await log.remove(e1);
		expect(
			await log.headsIndex.headsCache!.getCachedHeads()
		).to.have.members([e2.hash, e3.hash]);
		...
		for (const key of [...addedCacheKeys, ...removedCacheKeys]) {
			...
			expect(bytes && deserialize(bytes, HeadsCache)).equal(undefined);
		}
	});
	...
	it("resets heads eventually", async () => {
		...
		expect(cachedHeads).to.have.members(
			[
	...
	it("resets heads on load", async () => {
		...
		const cachedHeads = await log.headsIndex.headsCache!.getCachedHeads();
		expect(cachedHeads).to.have.members(entries.map((x) => x!.hash));
	...
	it("can cache heads concurrently", async () => {
		...
		expect(log.length).equal(entryCount);
		expect(await log.getHeads()).to.have.length(entryCount);
		const cachedHeads = await log.headsIndex.headsCache!.getCachedHeads();
		expect(cachedHeads).to.have.length(entryCount);
	});
	...
	it("resets heads when referencing all", async () => {
		...
		expect(await log.headsIndex.headsCache!.getCachedHeads()).to.have.length(3);
		const e4 = (await log.append({ data: 4 }, { meta: { next: entries } }))
			.entry;
		expect(await log.headsIndex.headsCache!.getCachedHeads()).to.have.length(1);
	...
	it("will load heads on write", async () => {
		...
		expect(log.values.length).equal(2);
	});
});
