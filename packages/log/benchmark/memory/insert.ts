...
try {
    ...
    process.on('message', async (message: Message) => {
        ...
        if (message.type === 'init') {
            ...
            store = new AnyBlockStore(await createStore(message.storage === 'in-memory' ? undefined : path.join("./tmp/blocks", uuid())));
            await log.open(store!, key, { indexer: await create(message.storage === 'in-memory' ? undefined : path.join("./tmp/index", uuid())) });
        }
