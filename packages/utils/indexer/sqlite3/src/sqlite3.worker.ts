...
class SqliteWorkerHandler {
    ...
    async onMessage(message: messages.DatabaseMessages | messages.StatementMessages) {
        if (message.type === 'create') {
            ...
            if (!db) {
                ...
                if (message.type === 'status') { return 'closed' }

                throw new Error('Database not found with id: ' + message.databaseId + ". For message type " + message.type);
            }
            if (message.type === 'exec') {
                ...
                if (message.type === 'bind') {
                    ...
                    return statement.get(message.values ? message.values.map(messages.resolveValue) : undefined);
                }
...
self.onmessage = async (messageEvent: MessageEvent<messages.DatabaseMessages | messages.StatementMessages>) => {
    try {
        const results = await worker.onMessage(messageEvent.data);
        self.postMessage({ type: 'response', id: messageEvent.data.id, result: results });
    } catch (error: any) {
        self.postMessage({ type: 'error', id: messageEvent.data.id, message: error?.message });
    }
}
