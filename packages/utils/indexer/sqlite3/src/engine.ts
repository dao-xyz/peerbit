...
export class SQLLiteIndex<T extends Record<string, any>> implements Index<T, any> {
	...
	id: string;
	constructor(
		readonly properties: { scope: string[], db: Database, schema: AbstractType<any>, start?: () => Promise<void> | void, stop?: () => Promise<void> | void },
		options?: { iteratorTimeout?: number }
	) {
		this.closed = true;
		this.id = uuid()
		this.scopeString = properties.scope.length > 0 ? "_" + escapePathToSQLName(properties.scope).join("_") : undefined;
	...
	async start(): Promise<void> {
		...
		const tables = getSQLTable(
			...
			getInlineTableFieldName(this.primaryKeyArr.slice(0, -1), this.primaryKeyArr[this.primaryKeyArr.length - 1]), // TODO fix this, should be array
			false,
			undefined,
			false
	...
	private async clearStatements() {
		...
		if (await this.properties.db.status() === 'closed') { // TODO this should never be true, but if we remove this statement the tests faiL for browser tests?
			return;
		}
	...
	async get(id: types.IdKey, options?: { shape: Shape }): Promise<IndexedResult<T> | undefined> {
		for (const table of this._rootTables) {
	...
	async put(value: T, _id: undefined): Promise<void> {
		...
		return insert(
			...
			value,
			this.tables,
			resolveTable(this.scopeString ? [this.scopeString] : [], this.tables, classOfValue, true),
