	...
	return {
		next: async (count: number) => {
			...
			if (!fetchedOnce) {
				fetchedOnce = true;
				query.fetch = count;
				res = await index.query(query, from);
			} else {
...
const search = <T>(
	...
	return index.query(query, from);
}


export const tests = (createIndicies: <T> (directory?: string) => Indices | Promise<Indices>, type: 'transient' | 'persist' = 'transient') => {
	...
	return describe("index", () => {
		...
		const setupDefault = async () => {
			// Create store
			const result = await setup({ schema: Document });
		...
		describe("indexBy", () => {
			const testIndex = async (
				...
				await store.put(doc);
				let result = await store.get(toId(doc[idProperty]));
				expect(result).to.exist;
				console.log(result, doc)
				checkDocument(result.value, doc);

				await store.del(toId(doc[idProperty]));
				expect(await store.getSize()).equal(0);
				result = await store.get(toId(doc[idProperty]));
			...
			describe('by decorator', () => {
				...
				it('can index by decorated id', async () => {
					...
					await testIndex(store, doc, getIdProperty(DocumentWithDecoratedId)[0]);
				});
			})
		});
		...
		describe("sort", () => {
			...
			describe("close", () => {
				...
				it("requires correct id", async () => {
					...
					const iterator = await iterate(store, request, iteratorOwner);
					expect(iterator.done()).to.be.false;
