	...
	implements types.Index<T, NestedType>
{
	...
	iterate<S extends types.Shape | undefined>(
		...
		const sort = query?.sort
			? Array.isArray(query.sort)
				...
				: [query.sort]
			: [];
		return query?.consistency === "weak" && sort.length === 0
			? this.weakUnsortedIterator(query, properties)
			: this.snapshotIterator(query, properties, sort);
	}

	private snapshotIterator<S extends types.Shape | undefined>(
		...
		const fetch = async (
			...
			if (!queue && !done) {
				...
				if (indexedDocuments.length > 1 && sort.length > 0) {
	...
	private weakUnsortedIterator<S extends types.Shape | undefined>(
		query: types.IterateOptions | undefined,
		properties: { shape?: S; reference?: boolean } | undefined,
	): types.IndexIterator<T, S> {
		const queryCoerced = types.toQuery(query?.query);
		...
		// observed. This path is explicit opt-in; the default iterator retains the
		// eager snapshot behavior above.
