...
describe("Sepck2561k1Keccak256", () => {
	...
	it('from PeerId', async () => {
		...
		const privateKey = new supportedKeys["secp256k1"].Secp256k1PrivateKey(peerId.privateKey!.slice(4))
		...
		expect(verifySignatureSecp256k1(signature, keypair.publicKey, data)).toBeTrue()
	})
