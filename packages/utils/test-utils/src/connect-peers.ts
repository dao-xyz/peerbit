...
const defaultFilter = (addr: { toString(): string }) =>
...
const connectIpfsNodes = async (
    ...
    const addresses1 = id1.addresses.filter((address) =>
        options.filter(address.toString())
    );
    const addresses2 = id2.addresses.filter((address) =>
        options.filter(address.toString())
    );
