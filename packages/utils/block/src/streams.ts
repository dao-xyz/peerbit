...
import { deserialize, field, fixedArray, serialize, vec } from '@dao-xyz/borsh'
...
export const ProtocolId = '/peerstream/1.0.0'
	...
	async publishMessage(from: PeerId, message: Message): Promise<void> {
		...
		peers.forEach(id => {
			...
			if (id.id.equals(from)) {
				logger.info('not sending message to sender' + id.id)
				return
			}

			logger.info('publish msgs on %p', id)
			if (!id.isWritable) {
				logger.error('Cannot send RPC to ' + id.id + ' as there is no open stream to it available',)
				return
			}
