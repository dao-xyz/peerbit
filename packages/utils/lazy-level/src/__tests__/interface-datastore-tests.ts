...
export function interfaceDatastoreTests(test) {
  ...
  describe('put', () => {
    ...
    it('parallel', async () => {
      ...
      const res = (await all(store.getMany(data.map(d => d.key)))).map(result => new Uint8Array(result))
  ...
  describe('getMany', () => {
    ...
    it('streaming', async () => {
      ...
      expect(new Uint8Array(res[0])).toEqual(uint8ArrayFromString('hello'))
    })
  ...
  describe('query', () => {
    ...
    tests.forEach(([name, query, expected]) => it(name, async () => {
      let res = (await all(store.query(query))).map(x => { return { key: x.key, value: new Uint8Array(x.value) } })
        ...
        if (query.orders == null) {
          ...
          res.forEach((r: any, i) => {
            ...
            if (r.value == null) {
              ...
              expect(new Uint8Array(r.value)).toEqual(exp[i].value)
            }
          })
        } else {
          res.forEach((r: any, i) => {
            expect(r.key.toString()).toEqual(expected[i].key.toString())
            if (r.value == null) {
              expect(expected[i].value).toBeUndefined()
            } else {
              expect(new Uint8Array(r.value)).toEqual(expected[i].value)
            }
          })
        }
    ...
    tests.forEach(([name, query, expected]) => it(name, async () => {
      ...
      if (Array.isArray(expected)) {
        if (query.orders == null) {
          ...
          res.forEach((r: any, i) => {
            expect(r.toString()).toEqual(expected[i].toString())
          })
        }
      } else if (typeof expected === 'number') {
        expect(res).toHaveLength(expected)
