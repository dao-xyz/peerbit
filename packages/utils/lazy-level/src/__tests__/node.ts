...
describe("LevelDatastore", () => {
	...
	it.skip("interop with go", async () => {
		const store = new LazyLevelDatastore(
			path.join(__dirname, "./tmp/test-repo", "datastore")
		);
		...
		for await (const e of store.query({})) {
			cids.push(
				CID.createV1(dagCbor.code, Digest.decode(e.key.uint8Array()))
			);
		}
	...
	it("should not leave iterators open and leak memory", (done) => {
		const cp = childProcess.fork(
			path.join(
				process.cwd(),
				"/test/fixtures/test-level-iterator-destroyx"
			),
			{ stdio: "pipe" }
		);
