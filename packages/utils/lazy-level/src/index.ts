...
export type LevelOptions = DatabaseOptions<string, Uint8Array> &
    OpenOptions & { batch?: LevelBatchOptions | boolean };
...
export class LazyLevelDatastore extends BaseDatastore {
    ...
    constructor(
        path: string | AbstractLevel<any, string, Uint8Array>,
        opts: LevelOptions = { batch: { interval: 300 } }
    ) {
        super();
    ...
    async get(key: Key) {
        ...
        try {
            const keyStr = key.toString();
            data =
                (this._tempStore && this._tempStore.get(keyStr)) ||
                (await this.db.get(keyStr, { valueEncoding: "view" }));
        } catch (err: any) {
