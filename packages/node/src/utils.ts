...
const parsePublicKey = (string: string): PublicSignKey | undefined => {
    ...
    else if (lc.startsWith('ed25519/') || lc.startsWith('solana/') || lc.startsWith('sol/')) {
        parsed = new Ed25519PublicKey({ publicKey: Buffer.from(lc.split('/')[1], 'hex') })
    }
    else {
        return;
    }
...
export const networkFromTopic = async (ipfs: IPFS, topic: string): Promise<TrustedNetwork | undefined> => {
    const publicKey = parsePublicKey(topic)
    if (publicKey) {
        let network = new TrustedNetwork({ rootTrust: publicKey })
        return network;
    }
    else {
        try {
            const loaded = await Program.load<TrustedNetwork>(ipfs, Address.parse(topic))
