...
import {
    Ed25519Keypair, SignatureWithKey,
...
describe("clock", () => {
    ...
    beforeAll(async () => {
        ...
        responder = new P({ clock: new ClockService({ trustedNetwork: new TrustedNetwork({ rootTrust: responderIdentity.publicKey }) }) });
        await responder.init(session.peers[0].ipfs, responderIdentity, {
            store: { resolveCache: () => Promise.resolve(new Cache(new MemoryLevel())), replicate: true } as any,
        } as any);
        reader = deserialize(serialize(responder), P);
        await reader.init(session.peers[1].ipfs, await createIdentity(), {
            store: { resolveCache: () => Promise.resolve(new Cache(new MemoryLevel())) } as any,
        } as any);
    ...
    it("signs and verifies", async () => {
        const entry = await Entry.create({
            ...
            ipfs: reader.ipfs,
            signers: [
                (async (data: Uint8Array) => new SignatureWithKey({ publicKey: reader.identity.publicKey, signature: await reader.identity.sign(data) })),
                reader.clock.sign.bind(reader.clock)
            ]
        })
        expect(entry.signatures.map(x => x.publicKey.hashCode())).toContainAllValues([reader.identity.publicKey.hashCode(), responder.identity.publicKey.hashCode()]);
