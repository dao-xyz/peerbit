...
describe("query", () => {
    ...
    it("any", async () => {
        const topic = uuid();
        await session.peers[0].ipfs.pubsub.subscribe(
            topic,
            async (msg: Message) => {
                ...
                ); // deserialize, so we now this works, even though we will not analyse the query
                await respond(
        ...
        await query(
            ...
            (resp) => {
                ...
                expect(
                    (r.results[0] as ResultWithSource).source
                ).toBeInstanceOf(NumberResult);
                results.push(
                    ((r.results[0] as ResultWithSource).source as NumberResult)
                        .number
                );
            },
            { waitForAmount: 1 }
    ...
    it("timeout", async () => {
        ...
        await waitFor(
            async () => (await session.peers[1].ipfs.pubsub.ls()).length == 0
        );
