				...
				if (q instanceof LogEntryEncryptionQuery) {
					if (entry._payload instanceof EncryptedThing) {
						...
						if (q.metadata.length > 0) {
							if (!check(entry._payload as EncryptedThing<any>, q.metadata)) {
								return false;
							}
						}

						if (q.next.length > 0) {
							if (!check(entry._payload as EncryptedThing<any>, q.next)) {
								return false;
							}
						}
						...
						if (q.signatures.length > 0) {
							if (!check(entry._payload as EncryptedThing<any>, q.signatures)) {
								return false;
							}
						}
					} else {
