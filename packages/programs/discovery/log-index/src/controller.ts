...
@variant(0)
export class LogEntryEncryptionQuery extends LogQuery implements EntryEncryptionTemplate<X25519PublicKey[], X25519PublicKey[], X25519PublicKey[]>  {
...
export class LogIndex extends ComposableProgram {
    ...
    _queryEntries(filter: ((entry: Entry<any>) => boolean)): Entry<any>[] {
        ...
        return this._store.oplog.values
            ...
            for (const q of query.queries) {
                if (q instanceof LogEntryEncryptionQuery) {
                    if (entry._payload instanceof EncryptedThing) {
                        const check = (encryptedThing: EncryptedThing<any>, keysToFind: X25519PublicKey[]) => {
                        ...
                        return q.signature.length == 0 && q.payload.length == 0 && q.clock.length == 0;
                    }
                }
                else {
