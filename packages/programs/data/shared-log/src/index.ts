...
@variant("shared_log")
export class SharedLog<T> extends ComposableProgram {
	@field({ type: Log })
	log: Log<T>;
	...
	async setup(options: LogProperties<T> & LogEvents<T> & SharedLogOptions) {
	...
	async open(
		...
		transport.services.pubsub.addEventListener(
			"subscribe",
		...
		await this.log.open(
			transport.services.blocks,
			getKeypairFromPeerId(transport.peerId),
			{
				...this._logProperties,
				...
				cache:
					transport.memory &&
					(await transport.memory.sublevel(sha256Base64Sync(this.log.id))),
			}
		);
		if (this._role instanceof Replicator) {
			...
			this.modifySortedSubscriptionCache(
				true,
				getPublicKeyFromPeerId(transport.peerId).hashcode()
			);
		} else {
			await this.log.load({ heads: true, reload: true });
		}

		return super.open(transport, options);
	}
