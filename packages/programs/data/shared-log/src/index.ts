...
const RECALCULATE_PARTICIPATION_DEBOUNCE_INTERVAL = 2500;
...
> extends Program<Args<T, D>, SharedLogEvents> {
	...
	private setupRebalanceDebounceFunction() {
		...
		this.rebalanceParticipationDebounced = debounceFixedInterval(
			...
			RECALCULATE_PARTICIPATION_DEBOUNCE_INTERVAL, // TODO make this dynamic on the number of replicators
		);
	}
	...
	private async updateOldestTimestampFromIndex() {
		...
		const oldestTimestampFromDB = (await iterator.next(1)).results[0]?.value
			.timestamp;
	...
	private async addReplicationRange(
		...
		const fn = async () => {
			...
			/* console.log(
				"DIFF",
				this.node.identity.publicKey.hashcode(),
				diffs.map((x) => x.range.hash.substring(0, 5) + " - " + x.type),
			); */
			...
			/* "3YUU2tgXPB1v7NMdPob37WDcixg4vi7qF1PkbSJFNc4=" ===
				this.node.identity.publicKey.hashcode() &&
				console.log("PEERS SIZE", await this.replicationIndex.getSize()); */
			return diffs;
		};
	...
	async append(
		...
		// TODO is it really necessary to call have this segment running for observers?
		// ---------------
		...
		for (const message of await createExchangeHeadsMessages(
			...
			if (options?.target === "replicators" || !options?.target) {
				if (message.heads[0].gidRefrences.length > 0) {
					...
					for (const ref of message.heads[0].gidRefrences) {
						...
						for (const entry of await entryFromGid.all()) {
							let coordinate = await this.getCoordinate(entry);
							if (coordinate == null) {
								coordinate = await this.createCoordinate(entry);
								console.error(
				...
				let map = this._gidPeersHistory.get(result.entry.meta.gid);
				if (!map) {
					map = new Map(leaders);
					this._gidPeersHistory.set(result.entry.meta.gid, map);
				} else {
					for (const [receiver, features] of leaders) {
						map.set(receiver, features);
					}
				}
	...
	async open(options?: Args<T, D>): Promise<void> {
		...
		} else if (hasIndexedReplicationInfo) {
			// emit change for out loaded segments
			// TODO check necessity of this call
			...
			if (msg instanceof ExchangeHeadsMessage) {
				...
				if (heads) {
					...
					for (const [gid, entries] of groupedByGid) {
						const fn = async () => {
							...
							const cursor = await this.createCoordinate(latestEntry);
							...
							if (!isLeader) {
								/* console.trace(
									...
									).has(this.node.identity.publicKey.hashcode()),
									this.isNotLeaderCount,
									this.node.identity.publicKey.hashcode(),
									entries[0].entry.hash,
									(
										await this.replicationIndex.query(new SearchRequest())
									).results.map((x) => x.value.toString()),
									(
										await this.replicationIndex.query(new SearchRequest())
							...
							if (isLeader) {
								let hasNonInterSecting = false;
								for (const [_, { intersecting }] of isLeader) {
									if (!intersecting) {
								...
								for (const entry of entries) {
									await this.persistCoordinate({
										assignedToRangeBoundary: hasNonInterSecting,
										coordinate: cursor,
										entry: entry.entry,
										replicators: isLeader.size,
										minReplicas: maxReplicasFromNewEntries,
									});
								}
								...
								if (fromIsLeader) {
									...
									peerSet.set(context.from!.hashcode(), fromIsLeader);
								}
				...
				for (const hash of msg.hashes) {
					...
					if (
						...
						// (this.node.identity.publicKey.hashcode() === "3YUU2tgXPB1v7NMdPob37WDcixg4vi7qF1PkbSJFNc4=") && console.log("RESPONSE I PRUNE", this.node.identity.publicKey.hashcode(), hasAndIsLeader);
						...
						// (this.node.identity.publicKey.hashcode() === "3YUU2tgXPB1v7NMdPob37WDcixg4vi7qF1PkbSJFNc4=") && console.log("ADD pendingIHave", this.node.identity.publicKey.hashcode(), hash);
						...
						if (prevPendingIHave) {
							...
							let timeout = setTimeout(() => {
								...
								// (this.node.identity.publicKey.hashcode() === "3YUU2tgXPB1v7NMdPob37WDcixg4vi7qF1PkbSJFNc4=") && console.log("Pending I Have delete", this.node.identity.publicKey.hashcode(), [...requesting], hash);
								...
								callback: async (entry: Entry<T>) => {
									if (
										...
										for (const peer of requesting) {
											...
											// 	(this.node.identity.publicKey.hashcode() === "3YUU2tgXPB1v7NMdPob37WDcixg4vi7qF1PkbSJFNc4=") && console.log("Callback pending I have", this.node.identity.publicKey.hashcode(), peer, hash);
										}
										...
										//	(this.node.identity.publicKey.hashcode() === "3YUU2tgXPB1v7NMdPob37WDcixg4vi7qF1PkbSJFNc4=") && console.log("Pending I have delete resolved", this.node.identity.publicKey.hashcode(), [...requesting], hash);

										this._pendingIHave.delete(hash);
									} else {
										// (this.node.identity.publicKey.hashcode() === "3YUU2tgXPB1v7NMdPob37WDcixg4vi7qF1PkbSJFNc4=") && console.log("Callback BUT not leader pending I have", this.node.identity.publicKey.hashcode(), [...requesting], hash);
									}
								},
							};
				...
				this.waitFor(context.from, {
					...
					.then(async () => {
						...
						/* "3YUU2tgXPB1v7NMdPob37WDcixg4vi7qF1PkbSJFNc4=" ===
							this.node.identity.publicKey.hashcode() &&
							console.log(
								"RECEIVED REPLICATION INFO",
								this.node.identity.publicKey.hashcode(),
								context.from!.hashcode(),
								replicationInfoMessage.segments.length,
							); */
	...
	private async findLeadersPersist(
		...
		const coordinate =
			options?.cursor != null
				? options.cursor
				: await this.createCoordinate(entry);
		const leaders = await this.findLeaders(coordinate, minReplicas, options);
		...
		if (isLeader || options?.persist) {
			...
			if (options?.persist?.prev) {
				...
				if (
					prev.assignedToRangeBoundary === hasNonInterSecting &&
					prev.coordinate === coordinate &&
					prev.replicators === leaders.size &&
					prev.replicasMissing === minReplicas - leaders.size
				) {
	...
	async isLeader(
		entry: ShallowOrFullEntry<any> | number,
		...
		const coordinate =
			typeof entry === "number" ? entry : await this.createCoordinate(entry);
	...
	private async waitForIsLeader(
		...
		return new Promise((resolve, reject) => {
			...
			const check = () =>
				this.findLeaders(cursor, numberOfLeaders).then((leaders) => {
	...
	async findLeaders(
		cursor: number | ShallowOrFullEntry<any> | EntryReplicated,
		numberOfLeaders: number,
		options?: {
			roleAge?: number;
		},
		...
		const coordinate =
			typeof cursor === "number" ? cursor : await this.createCoordinate(cursor);
		const leaders = await this.findLeadersFromU32(
			coordinate,
			numberOfLeaders,
			options,
		);
	...
	private async groupByLeaders(
		cursors: number[] | (ShallowOrFullEntry<any> | EntryReplicated)[],
		numberOfLeaders: number,
		options?: {
			roleAge?: number;
		},
		...
		const leaders = await Promise.all(
			cursors.map((x) => this.findLeaders(x, numberOfLeaders, options)),
		);
	...
	private createCoordinate(entry: ShallowOrFullEntry<any> | EntryReplicated) {
		return this.domain.fromEntry(entry);
	}

	private async getCoordinate(entry: { hash: string }) {
		const result = await this.entryCoordinatesIndex
			.iterate({ query: { hash: entry.hash } })
			.all();
		return result[0]?.value.coordinate;
	}
	private async persistCoordinate(properties: {
		...
		await this.entryCoordinatesIndex.put(
			new EntryReplicated({
				...
				replicators: properties.replicators,
			}),
		);
	...
	prune(
			...
			this._pendingDeletes.set(entry.hash, {
				...
				resolve: async (publicKeyHash: string) => {
					...
					const leaders = await this.waitForIsLeader(
						cursor ?? (cursor = await this.createCoordinate(entry)),
						minMinReplicasValue,
						publicKeyHash,
					);
					if (leaders) {
						...
						if (minMinReplicasValue <= existCounter.size) {
							...
							this.log
								...
								.then(() => {
									/* "3YUU2tgXPB1v7NMdPob37WDcixg4vi7qF1PkbSJFNc4=" ===
										this.node.identity.publicKey.hashcode() &&
										console.log("DELETED", this.log.length, entry.hash); */
									deleted.add(entry.hash);
									return resolve();
								})
								.catch((e: any) => {
									console.error("FIALED TO DELETE");
									reject(new Error("Failed to delete entry: " + e.toString()));
								});
						}
					} else {
						// console.log("NOT DELEATING BC IS NOT LEADER", publicKeyHash, entry.hash, (await this.replicationIndex.query(new SearchRequest())).results.map((x) => x.value.toString()));
					}
				},
			});
		...
		this.groupByLeaders(
			filteredEntries,
			maxReplicas(this, filteredEntries),
		).then((map) => {
			...
			if (e.detail.publicKey.equals(this.node.identity.publicKey) === false) {
				const peerEntries = (
					await this.groupByLeaders(
						filteredEntries.filter((x) => !deleted.has(x.hash)),
						maxReplicas(this, filteredEntries),
					)
	...
	async getPrunable() {
		...
		for (const head of heads) {
			const isLeader = await this.isLeader(head, maxReplicas(this, [head]));
	...
	async onReplicationChange(
		...
		// console.log("CHANGE", this.node.identity.publicKey.hashcode(), (Array.isArray(changeOrChanges) ? changeOrChanges : [changeOrChanges]).map(x => x));
		...
		try {
			...
			//("rRcHKy8yCun+32/dvRAkNMqvmXVb/N/X3Sis/wkDxKQ=" === this.node.identity.publicKey.hashcode()) && console.log("REBALANCE", this.node.identity.publicKey.hashcode(), change.length, change[0].range.toString());
			...
			for await (const { gid, entries: coordinates } of toRebalance(
				...
				rbCount += coordinates.length;

				const { leaders: currentPeers, isLeader } =
					await this.findLeadersPersist(
						coordinates[0], // TODO can hashes have different coordinates?
						maxReplicas(this, coordinates), // pick max replication policy of all entries, so all information is treated equally important as the most important
						{
							cursor: coordinates[0].coordinate, // TODO can hashes have different coordinates?
							persist: {
								prev: coordinates[0],
							},
						},
				...
				const currentPeersSet = new Map(currentPeers);
			...
			/* "3YUU2tgXPB1v7NMdPob37WDcixg4vi7qF1PkbSJFNc4=" ===
				this.node.identity.publicKey.hashcode() &&
				console.log("REBALANCE COUNT:", rbCount);
 */
	...
	async rebalanceParticipation() {
		...
		const fn = async () => {
			...
			if (this._isAdaptiveReplicating) {
				...
				/* (this.node.identity.publicKey.hashcode() === "rRcHKy8yCun+32/dvRAkNMqvmXVb/N/X3Sis/wkDxKQ=") && console.log("FACTOR DIFF?", this.node.identity.publicKey.hashcode(), relativeDifference > 0.0001, newFactor, relativeDifference, {
					usedMemory,
					currentFactor: dynamicRange.widthNormalized,
					totalFactor: totalParticipation, // TODO use this._totalParticipation when flakiness is fixed
					peerCount: peersSize,
