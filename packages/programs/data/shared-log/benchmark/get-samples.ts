...
let create = async (...rects: ReplicationRangeIndexable[]): Promise<[Index<ReplicationRangeIndexable, unknown>, any]> => {
...
for (let i = 0; i < rangeCount; i++) {
	ranges.push(...[
		new ReplicationRangeIndexable({ publicKey: a, length: 0.2 / rangeCount, offset: (0 + rangeCount / i) % 1, timestamp: 0n }),
		new ReplicationRangeIndexable({ publicKey: b, length: 0.4 / rangeCount, offset: (0.333 + rangeCount / i) % 1, timestamp: 0n }),
		new ReplicationRangeIndexable({ publicKey: c, length: 0.6 / rangeCount, offset: (0.666 + rangeCount / i) % 1, timestamp: 0n }),
		new ReplicationRangeIndexable({ publicKey: c, length: 0.6 / rangeCount, offset: (0.666 + rangeCount / i) % 1, timestamp: 0n })
	])
}

const [index, indices] = await create(
	...ranges
);
