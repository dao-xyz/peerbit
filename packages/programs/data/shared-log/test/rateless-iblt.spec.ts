...
describe("rateless-iblt-syncronizer", () => {
	...
	// New test to ensure resources are freed after synchronization completes
	it("frees encoder/decoder resources on close", async () => {
		// Set up logs with enough data to trigger a sync process
		const syncedCount = 100;
		const unsyncedCount = 50;
		await setupLogs(syncedCount, unsyncedCount);

		// Dial nodes to start syncing
		await db1.node.dial(db2.node.getMultiaddrs());

		// Wait for synchronization to complete (for testing purposes,
		// we wait until both logs reach the same length)
		await waitForResolved(
			() => expect(db1.log.log.length).to.equal(db2.log.log.length),
			{ timeout: 20000 },
		);

		// Assume the synchronizer is accessible via db1.log.synchronizer
		const synchronizer = db1.log.syncronizer as RatelessIBLTSynchronizer<any>;

		// Spy on the free methods of each sync process (both ingoing and outgoing)
		synchronizer.ingoingSyncProcesses.forEach((process) => {
			sinon.spy(process, "free");
		});
		synchronizer.outgoingSyncProcesses.forEach((process) => {
			sinon.spy(process, "free");
		});

		// Close the synchronizer to trigger resource cleanup
		await synchronizer.close();

		// Verify that the free functions were called for each active process
		synchronizer.ingoingSyncProcesses.forEach((process) => {
			const sinonProxy = process.free as sinon.SinonSpy;
			expect(sinonProxy.called).to.be.true;
		});
		synchronizer.outgoingSyncProcesses.forEach((process) => {
			const sinonProxy = process.free as sinon.SinonSpy;
			expect(sinonProxy.called).to.be.true;
		...
		// And finally, ensure that the process maps are empty
		expect(synchronizer.ingoingSyncProcesses.size).to.equal(0);
		expect(synchronizer.outgoingSyncProcesses.size).to.equal(0);
	});
});
