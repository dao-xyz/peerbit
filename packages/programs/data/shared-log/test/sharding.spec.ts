		...
		describe(`sharding`, () => {
			...
			it("will not have any prunable after balance", async () => {
				...
				await waitForResolved(async () => {
					console.log(db2.log.log.length);
			...
			it("distributes to leaving peers", async () => {
				...
				try {
					...
					console.log(
						(
							await Promise.all(
								[db1, db2, db3].map((db) => db.log.getMyReplicationSegments()),
							)
						).map((x) => x[0].widthNormalized),
					);
					console.log(
						(
							await Promise.all(
								[db1, db2, db3].map((db) => db.log.getPrunable()),
							)
						).map((x) => x.length),
					);
					...
					console.log(
						await Promise.all(
							[db1, db2, db3].map(
								async (x) =>
									(await x.log.log.entryIndex.iterate([]).all()).length,
							),
						),
					);
				...
				try {
					...
					console.log(
						(
							await Promise.all([db1, db2].map((db) => db.log.getPrunable()))
						).map((x) => x.length),
					);
					console.log(
						(
							await Promise.all(
								[db1, db2].map((db) => db.log.getMyReplicationSegments()),
							)
						).map((x) => x[0].widthNormalized),
					);
					throw error;
			...
			describe("distribution", () => {
				...
				describe("fixed", () => {
					it("can weight by factor", async () => {
						...
						await waitForResolved(
							() =>
								expect(db2.log.log.length).greaterThan(db1.log.log.length + 20),
							{
								timeout: 3e4,
							},
						);
