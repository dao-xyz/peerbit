...
describe("raw exchange-head sync", () => {
	...
	it("reuses full-retain native raw receive selection plans", async () => {
		const session = await TestSession.disconnected(2, {
			indexer: (directory) => createRustIndexer(directory),
		});
		...
		try {
			...
			for (let i = 0; i < entries.length; i++) {
				for (const [coordinateIndex, coordinate] of backbone
					...
					backbone.putRange({
						id: `target-retain-${i}-${coordinateIndex}`,
						hash: targetHash,
						timestamp: 0,
						start1: start,
						end1: start + 1n,
			...
			const groupLeaderSpy = sinon.spy(
				backbone,
				"planPreparedRawReceiveGroupLeaders",
			);
			try {
				...
				expect(groupLeaderSpy.callCount).to.equal(0);
				const receivePlanProfile = profileEvents.find(
					(event) => event.name === "sharedLog.receive.plan",
				);
				expect(
