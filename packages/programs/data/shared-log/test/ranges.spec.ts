...
import { type ReplicatorRect } from "../src/replication.js";
import { Ed25519Keypair } from "@peerbit/crypto";

import { Replicator } from "../src/role.js";
import { containsPoint, getCover, getSamples } from "../src/ranges.js";
...
describe("ranges", () => {
            ...
            describe('rotation: ' + String(rotation), () => {
                ...
                describe('underflow', () => {
                    it('includes all', () => {
                        ...
                        expect([...getCover(1, peers, 1e5, a)]).to.have.members([a.hashcode(), b.hashcode(), c.hashcode()])
                    })
                })
                ...
                describe("overflow", () => {
                    it("local first", () => {
                        ...
                        // we try to cover 0.5 starting from a
                        // this should mean that we would want a and b, because c is not mature enough, even though it would cover a wider set
                        expect([...getCover(1, peers, 1e5, a)]).to.have.members([a.hashcode()])
                    })
                })
                ...
                describe("skip", () => {
                    it('next', async () => {
                        ...
                        // we try to cover 0.5 starting from a
                        // this should mean that we would want a and b, because c is not mature enough, even though it would cover a wider set
                        expect([...getCover(0.5, peers, 1e5, a)]).to.have.members([a.hashcode(), b.hashcode()])
                    })
                    it('between', async () => {
                ...
                describe("boundary", () => {
                    ...
                    it('after', () => {
                        ...
                        expect([...getCover(0.6, peers, 0, b)]).to.have.members([b.hashcode()])
                    })
                    ...
                    it('all if non matured', () => {
                        ...
                        expect([...getCover(0.5, peers, 1e5, b)]).to.have.members([a.hashcode(), b.hashcode(), c.hashcode()])
                    })
                })
            })
    ...
    describe("getSamples", () => {
        ...
        rotations.forEach((rotation) => {
            it("will get at least amount of samples: " + rotation, async () => {
                ...
                expect(getSamples(0.78, peers, 2, 0)).to.have.length(2)
            })
        ...
        it("factor 0 ", async () => {
            ...
            expect(getSamples(0.37, peers, 2, 0)).to.have.members([b, c].map(x => x.hashcode()))
        })
        ...
        it("factor 0 with 3 peers factor 1", async () => {
            ...
            expect(getSamples(0.937, peers, 2, 0)).to.have.members([a, c].map(x => x.hashcode()))
        })
        ...
        describe('maturity', () => {
            it("starting at unmatured", async () => {
                ...
                expect(getSamples(0.7, peers, 2, 1e5)).to.have.members([a, b, c].map(x => x.hashcode()))
            })
            ...
            it("starting at matured", async () => {
                ...
                expect(getSamples(0.5, peers, 2, 1e5)).to.have.members([a, c].map(x => x.hashcode()))
            })
            ...
            it("starting at matured-2", async () => {
                ...
                expect(getSamples(0, peers, 2, 1e5)).to.have.members([a, c].map(x => x.hashcode()))
            })
        })
    })
    ...
    describe("containsPoint", () => {
        it("length 0", () => {
            expect(containsPoint({ factor: 0, offset: 0 }, 0)).to.be.false;
            expect(containsPoint({ factor: 0, offset: 0 }, 0.1)).to.be.false;
            expect(containsPoint({ factor: 0, offset: 0.1 }, 0)).to.be.false;
        });

        it("length 1", () => {
            expect(containsPoint({ factor: 1, offset: 0 }, 0)).to.be.true;
            expect(containsPoint({ factor: 1, offset: 0 }, 0.1)).to.be.true;
            expect(containsPoint({ factor: 1, offset: 0.1 }, 0)).to.be.true;
        });


        it("wrapped", () => {
            expect(containsPoint({ factor: 0.3, offset: 0.8 }, 0.1)).to.be.true;
            expect(containsPoint({ factor: 0.3, offset: 0.8 }, 0.2)).to.be.false;
        });

        it("unwrapped", () => {
            expect(containsPoint({ factor: 0.1, offset: 0.8 }, 0.89)).to.be.true;
            expect(containsPoint({ factor: 0.1, offset: 0.8 }, 0.91)).to.be.false;
        });
    });
})
