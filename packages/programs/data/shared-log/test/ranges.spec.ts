...
const getSamples = async (
	...
	const map = await getSamplesMap(offset, peers, count, roleAge);
	return [...map.keys()];
};
        ...
        rotations.forEach((rotation) => {
            ...
            it("evenly distributed: " + rotation, async () => {
                ...
                for (let i = 0; i < count; i++) {
                    const leaders = await getSamplesMap(scaleToU32(i / count), peers, 1, 0)
	...
	describe("toRebalance", () => {
		...
		rotations.forEach((rotation) => {
			...
			describe("rotation: " + String(rotation), () => {
				...
				it("not enoughly replicated", async () => {
					await create(
						new EntryReplicated({
							coordinate: scaleToU32(rotate(0)),
							assignedToRangeBoundary: false,
					...
					expect(result.map((x) => x.gid)).to.deep.eq(["a"]);
				});

				it("enoughly replicated after change", async () => {
					await create(
						new EntryReplicated({
							coordinate: scaleToU32(rotate(0)),
							assignedToRangeBoundary: false,
