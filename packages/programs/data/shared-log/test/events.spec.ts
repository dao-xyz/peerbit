...
describe("events", () => {
	...
	it("fences entry replication before the mutation lane across reopen", async () => {
		...
		try {
			...
			const oldOwnershipLifecycle = log._instanceLifecycle?.ownershipLifecycleController;
			log.poisonReplicationOwnership(new Error("forced ownership poison"));
			await db.close();
			await session.peers[0].open(db, {
				args: { replicate: false, timeUntilRoleMaturity: 0 },
	...
	it("fences a blocked replication announcement from reopened repair queues", async () => {
		...
		const ownershipLifecycleController = log._instanceLifecycle?.ownershipLifecycleController;
	...
	it("does not let a stale announcement retry poison reopened ownership", async () => {
		...
		try {
			const retrying = log._announcements.retryCurrentReplicationStateAnnouncement();
			...
			const reopenedLifecycle = log._instanceLifecycle?.ownershipLifecycleController;

			releaseSnapshotRead.resolve();
			await retrying;

			expect(log._replicationRangeMutationFailure).to.be.undefined;
			expect(log._instanceLifecycle?.ownershipLifecycleController).to.equal(reopenedLifecycle);
	...
	it("does not let a stale announcement repair poison reopened ownership", async () => {
		...
		try {
			const repairing = log._announcements.runCurrentReplicationStateAnnouncementRepair();
			await snapshotReadStarted.promise;
			getMyReplicationSegments.restore();
	...
	it("fences a blocked repair runner from poison and reopened frontier state", async () => {
		...
		try {
			...
			const oldRepairLifecycle = log._instanceLifecycle?.ownershipLifecycleController;
			log.poisonReplicationOwnership(new Error("forced ownership poison"));
	...
	it("fences a reconnect until an in-flight unsubscribe removal is coherent", async () => {
		...
		try {
			...
			expect(log._liveness._replicatorLastActivityAt.has(remoteHash)).to.be.true;
			...
			expect(log._liveness._replicatorLastActivityAt.has(remoteHash)).to.be.false;
