...
describe("replication degree", () => {
	...
	it("prune on insert many", async () => {
		...
		console.log(db1.node.identity.publicKey.hashcode());
		/* await delay(2e4) */
	...
	describe("commit options", () => {
		...
		it("will index replication underflow degree", async () => {
			...
			try {
				expect(indexedDb1.map((x) => x.value.replicasMissing)[0]).to.equal(3);

				expect(indexedDb1.map((x) => x.value.replicators)[0]).to.equal(1);

				expect(indexedDb2.map((x) => x.value.replicasMissing)[0]).to.equal(3);
				expect(indexedDb2.map((x) => x.value.replicators)[0]).to.equal(1);
			} catch (error) {
				throw error;
			}
		});
	...
	describe("update", () => {
		...
		it("close", async () => {
			...
			console.log("-----------------------");
			[db1, db2].map(x => x.log.useFiltering = false);
			await db3.close();
			try {
				await checkBounded(entryCount, 1, 1, db1, db2);
			} catch (error) {
				const size = await getUnionSize([db1, db2], entryCount);
				console.log("???", size)
				throw error
			}
		});
