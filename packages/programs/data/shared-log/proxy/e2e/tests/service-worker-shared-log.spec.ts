...
test.describe("shared-log canonical service worker", () => {
	test("shares shared-log across tabs", async ({ page }, testInfo) => {
		const session = sessionParam(`shared-log-${testInfo.title}`);
		await page.goto(
			`/?scenario=shared-log&label=tab-1&session=${session}&transport=serviceworker`,
		);
		...
		await page2.goto(
			`/?scenario=shared-log&label=tab-2&session=${session}&transport=serviceworker`,
		);
