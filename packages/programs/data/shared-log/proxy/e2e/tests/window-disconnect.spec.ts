...
test.describe("canonical window disconnects", () => {
	test("fails RPC calls after host disconnects clients", async ({
		...
		await page.evaluate(
			({ session, channel }) => {
				window.open(
					`/?scenario=shared-log&label=popup&session=${session}&transport=window&role=child&channel=${channel}`,
					"canonical-popup",
				);
			},
			{ session, channel },
		...
		const result = await child.evaluate(async () => {
			try {
				await (window as any).__canonicalTest.refresh();
	...
	test("fails RPC calls after abrupt host close (no goodbyes)", async ({
		...
		await host.evaluate(
			({ session, channel }) => {
				window.open(
					`/?scenario=shared-log&label=popup&session=${session}&transport=window&role=child&channel=${channel}&keepAliveIntervalMs=250&keepAliveTimeoutMs=500&keepAliveFailures=1`,
					"canonical-popup",
				);
			},
			{ session, channel },
		...
		const result = await child.evaluate(async () => {
			try {
				await Promise.race([
					(window as any).__canonicalTest.refresh(),
