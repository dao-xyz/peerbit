...
const getDocStats = async (page: { evaluate: any }) => {
	const stats = await page.evaluate(() =>
		(window as any).__canonicalTest.getHostStats(),
	);
	const entries = stats?.documents?.entries ?? [];
	const total =
		typeof stats?.documents?.total === "number"
...
test.describe("canonical lifetime", () => {
	...
	test("releases document refs when tabs close", async ({ page }, testInfo) => {
		...
		await expect
			.poll(() => getDocStats(page3), { timeout: 20_000 })
	...
	test("releases document refs after abrupt tab close", async ({
		page,
	}, testInfo) => {
		const session = sessionParam(`lifetime-abrupt-${testInfo.title}`);
		await page.goto(
			`/?scenario=documents&label=tab-1&session=${session}&autoClose=false`,
		);
		await expect(page.getByTestId("status")).toHaveText("ready", {
			timeout: 20_000,
		});
		...
		const page2 = await page.context().newPage();
		await page2.goto(
			`/?scenario=documents&label=tab-2&session=${session}&autoClose=false`,
		);
		await expect(page2.getByTestId("status")).toHaveText("ready", {
			timeout: 20_000,
		});
