...
describe("query", () => {
  ...
  beforeEach(async () => {
    ...
    await observerStore.init(observer, await createIdentity(), {
      replicationTopic,
      store: { ...DefaultOptions, resolveCache: () => new Cache(cacheStore2) },
    });
  });
  afterEach(async () => {
  ...
  it("only context", async () => {
    ...
    await writeStore.add(
      "world",
      new Range({ offset: BigInt("hello ".length), length: "world".length })
    );
  ...
  it("match part", async () => {
    ...
    expect((result.coordinates as RangeCoordinates).coordinates).toHaveLength(
      2
    );
    expect(
