...
Object.keys(testAPIs).forEach(API => {
  describe(`orbit-db - Feed Load Amount (${API})`, function () {
    ...
    describe('Feed Load Amount', function () {
      ...
      test('add 10 items and verify they are in the index', async () => {
      ...
      test('reopen store and load 10 items', async () => {
      ...
      test('reopen store and load 1 item more than exists', async () => {
        await db.close()
        db = await orbitdb1.open(address)
      ...
      test('reopen store and load 5 item more than exists', async () => {
      ...
      test('reopen store and load 20 items more than exists', async () => {
