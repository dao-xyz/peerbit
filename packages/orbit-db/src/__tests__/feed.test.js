...
Object.keys(testAPIs).forEach(API => {
  describe(`orbit-db - Feed Database (${API})`, function () {
    ...
    describe('Feed', function () {
      ...
      test('returns 0 items when it\'s a fresh database', async () => {
        const db = await orbitdb1.feed('feed database')
      ...
      test('returns the added entry\'s hash, 1 entry', async () => {
      ...
      test('returns the added entry\'s hash, 2 entries', async () => {
      ...
      test('adds an item that is > 256 bytes', async () => {
      ...
      test('deletes an item when only one item in the database', async () => {
      ...
      test('deletes an item when two items in the database', async () => {
      ...
      test('deletes an item between adds', async () => {
    ...
    describe('Iterator', function () {
      ...
      describe('Defaults', function () {
        ...
        test('returns an item with the correct structure', () => {
          const iter = db.iterator()
          const next = iter.next().value
          assert.notEqual(next, null)
        ...
        test('returns 1 item as default', () => {
          const iter = db.iterator()
        ...
        test('returns items in the correct order', () => {
          const amount = 3
      ...
      describe('Options: limit', function () {
        test('returns 1 item when limit is 0', () => {
        ...
        test('returns 1 item when limit is 1', () => {
          const iter = db.iterator({ limit: 1 })
          const first = iter.next().value
          const second = iter.next().value
          assert.equal(first.hash, last(hashes))
        ...
        test('returns all items when limit is bigger than -1', () => {
        ...
        test('returns all items when limit is bigger than number of items', () => {
          const messages = db.iterator({ limit: 300 })
            .collect()
            .map((e) => e.hash)
      ...
      describe('Option: ranges', function () {
        describe('gt & gte', function () {
          test('returns 1 item when gte is the head', () => {
          ...
          test('returns 0 items when gt is the head', () => {
            const messages = db.iterator({ gt: last(hashes) }).collect()
            assert.equal(messages.length, 0)
          })

          test('returns 2 item when gte is defined', () => {
          ...
          test('returns all items when gte is the root item', () => {
          ...
          test('returns items when gt is the root item', () => {
          ...
          test('returns items when gt is defined', () => {
            const messages = db.iterator({ limit: -1 })
              .collect()
              .map((e) => e.hash)
        ...
        describe('lt & lte', function () {
          test('returns one item after head when lt is the head', () => {
          ...
          test('returns all items when lt is head and limit is -1', () => {
          ...
          test('returns 3 items when lt is head and limit is 3', () => {
          ...
          test('returns null when lt is the root item', () => {
            const messages = db.iterator({ lt: hashes[0] }).collect()
            assert.equal(messages.length, 0)
          })

          test('returns one item when lte is the root item', () => {
          ...
          test('returns all items when lte is the head', () => {
          ...
          test('returns 3 items when lte is the head', () => {
