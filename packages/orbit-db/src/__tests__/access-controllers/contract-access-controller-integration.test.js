    ...
    describe('OrbitDB Integration', function () {
      accessControllers.forEach(async (ac, i) => {
        ...
        describe('access controls', () => {
          test('throws error if key not permitted to write', async () => {
            let err
            try {
          ...
          test('can\'t grant access if not admin', async () => {
          ...
          test('can\'t revoke access if not admin', async () => {
          ...
          test('can check permissions without defaultAccount set', async () => {
            db2.access.defaultAccount = null
            const canAppend = await db2.access.canAppend({ identity: id1 }, id1.provider)
            assert.strictEqual(canAppend, true)
          })

          test('can\'t change permissions without from address if no defaultAccount set', async () => {
            let err
            db2.access.defaultAccount = null
            try {
              await db2.access.grant('write', id2.id)
          ...
          test('can change permissions by passing in from address', async () => {
            let err
            db2.access.defaultAccount = null
            try {
