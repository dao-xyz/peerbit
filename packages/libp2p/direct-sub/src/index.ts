...
import { DataMessage, DirectStream, DirectStreamOptions, Message, PeerStreams } from '@dao-xyz/libp2p-direct-stream'
import errcode from 'err-code'
import { PubSubMessage, Subscribe, PubSubData, toUint8Array, Unsubscribe, GetSubscribers } from './encoding.js'
...
export class DirectSub extends DirectStream<PubSubEvents> {
	...
	public async onPeerConnected(peerId: PeerId, conn: Connection) {
		...
		this.requestSubscribers([...this.topics.keys()], [this.peers.get(getPublicKeyFromPeerId(peerId).hashcode())!])
		return ret;
	}
	...
	subscribe(topic: string) {
		...
		if (!this.subscriptions.has(topic)) {
			...
			this.publishMessage(this.libp2p.peerId, new DataMessage({ data: toUint8Array((new Subscribe({ topics: [topic] })).serialize()) }).sign(this.sign))
		}
	...
	unsubscribe(topic: string) {
		...
		logger.info(`unsubscribe from ${topic} - am subscribed ${wasSubscribed}`)

		if (wasSubscribed) {
			this.subscriptions.delete(topic)
			this.topics.delete(topic);
			this.topicsToStreams.delete(topic);
			this.publishMessage(this.libp2p.peerId, new DataMessage({ data: toUint8Array((new Unsubscribe({ topics: [topic] })).serialize()) }).sign(this.sign))
		}
	}
	...
	requestSubscribers(topic: string | string[], streams?: PeerStreams[]): Promise<void> {
		if (!this.started) {
			throw errcode(new Error('not started yet'), 'ERR_NOT_STARTED_YET')
		}
		...
		return this.publishMessage(this.libp2p.peerId, new DataMessage({ data: toUint8Array((new GetSubscribers({ topics })).serialize()) }).sign(this.sign), streams);
	}
	...
	getNeighboursWithTopics(topics: string[], otherPeers?: string[]): Set<string> {
	...
	getStreamsWithTopics(topics: string[], otherPeers?: string[]): PeerStreams[] {
		...
		return [...this.peers.values()].filter(s => peers.has(s.publicKey.hashcode()));
	}



	async publish(data: Uint8Array, options: { topics: string[], to?: (string | PeerId | PublicSignKey)[] } | { to: (string | PeerId | PublicSignKey)[] }): Promise<void> {

		const topics = (options as { topics: string[] }).topics?.map(x => x.toString()) || [];
		const tos = options?.to?.map(x => x instanceof PublicSignKey ? x.hashcode() : (typeof x === 'string' ? x : getPublicKeyFromPeerId(x).hashcode())) || [];
		// Embedd topic info before the data so that peers/relays can also use topic info to route messages efficiently 
		const message = new PubSubData({ topics: topics.map(x => x.toString()), data });
		const bytes = message.serialize();
		await super.publish(bytes instanceof Uint8Array ? bytes : bytes.subarray(), { to: this.getNeighboursWithTopics(topics, tos) })

	}

	async onDataMessage(from: PeerId, stream: PeerStreams, message: DataMessage) {
		const pubsubMessage = PubSubMessage.deserialize(message.data instanceof Uint8Array ? new Uint8ArrayList(message.data) : message.data);
		if (pubsubMessage instanceof PubSubData) {
			...
			/* 
				const peersOnTopics = this.getPeersOnTopics(topics, message.to);

			if (message.to.length === peersOnTopics.length) {
				// do nothing
				// don't even verify for the performance penalty, if we forward the reciever will verify
				// if we will dispatch event, we will however verify
			}
			else {
				if (!message.verify()) {
				...
				message.to = [...peersOnTopics]
			} */

			const isFromSelf = this.libp2p.peerId.equals(from)
			if (!isFromSelf || this.emitSelf) {
				//	const isForAll = message.to.length === 0;
				const isForMe = pubsubMessage.topics.find(topic => this.subscriptions.has(topic))/*  && !isForAll */ || message.to.find(x => this.publicKeyHash === x)
				if (isForMe) {
			...
			// Forward
			const streamToSendTO = this.getStreamsWithTopics(pubsubMessage.topics, message.to);
			await this.relayMessage(from, message, streamToSendTO.length > 0 ? streamToSendTO : undefined) // if not find any stream, send to all
			...
			pubsubMessage.topics.forEach((topic) => {
				...
				let key = subscriber.hashcode(); // Assume first signature is the one who is signing
				...
				const fromPublicKeyHash = getPublicKeyFromPeerId(from).hashcode();
				//this.streamToTopics.get(from.toString())!.add(topic);
				this.topicsToStreams.get(topic)!.add(fromPublicKeyHash)

			});

			this.dispatchEvent(new CustomEvent('subscribe', { detail: pubsubMessage }));
			...
			this.dispatchEvent(new CustomEvent('unsubscribe', { detail: pubsubMessage }));
			...
			let myTopics = pubsubMessage.topics.filter(topic => this.subscriptions.has(topic))
			if (myTopics.length > 0) {
				...
				this.publishMessage(this.libp2p.peerId, new DataMessage({ data: toUint8Array((new Subscribe({ topics: myTopics })).serialize()) }).sign(this.sign), [stream]); // send back to same stream
			}
