...
export class Routes {
	...
	addLink(from: string, to: string, origin: string = this.peerId): string[] {
		...
		if (newReachableNodesFromOrigin.length === 1 && this.linksCount >= 2 && linkExisted === false) {
			const t = 123;
		}
	...
	deleteLink(from: string, to: string, origin: string = this.peerId): string[] {
		...
		if (link) {
			...
			if (fromWasReachable && origin !== from && this.getPath(origin, from).length === 0) {
				unreachableNodesFromOrigin.push(from);
			}
			if (toWasReachable && origin !== to && this.getPath(origin, to).length === 0) {
			...
			for (const disconnected of [...unreachableNodesFromOrigin]) {
				...
				while (stack.length > 0) {
					...
					if (this.graph.removeNode(node.id) && disconnected !== node.id.toString()) {
