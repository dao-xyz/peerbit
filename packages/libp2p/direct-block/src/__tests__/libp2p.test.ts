...
describe("transport", function () {
	...
	it("can restart", async () => {
		...
		await store(session, 1).close();

		await store(session, 0).open();
		await store(session, 1).open();
		await waitForPeers(store(session, 0), store(session, 1));
	});
	...
	it("rw", async () => {
		...
		const cid = await store(session, 0).put(await createBlock(data, "raw"));

		expect(stringifyCid(cid)).toEqual(
			"zb2rhbnwihVzMMEGAPf9EwTZBsQz9fszCnM4Y8mJmBFgiyN7J"
		);
		const readData = await store(session, 1).get<Uint8Array>(stringifyCid(cid));
	...
	it("read concurrently", async () => {
		...
		for (let i = 0; i < 100; i++) {
			promises.push(store(session, 1).get<Uint8Array>(stringifyCid(cid)));
		}
	...
	it("reads from joining peer", async () => {
		...
		const readDataPromise = store(session, 1).get<Uint8Array>(
			stringifyCid(cid)
		);
	...
	it("timeout", async () => {
		...
		const t1 = +new Date();
		const readData = await store(session, 0).get<Uint8Array>(
	...
	/* it('can handle conurrent read/write', async () => {
		...
		for (let i = 0; i < count; i++) {
			const p = async () => {
				...
				const readData = await sx(session,1).get<Uint8Array>(stringifyCid(cid));
