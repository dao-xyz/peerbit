 ...
 *
 * `subscribe()` debounces via `debounceSubscribeAggregator`.  The actual
 * `_subscribe()` handler (which calls `initializeTopic`) fires only after the
 * debounce window.  If a remote Subscribe message arrives in that window, the
 * handler finds `this.topics.get(topic) === undefined` and silently drops the
 * remote subscription.
 *
 * Fix: eagerly call `initializeTopic(topic)` inside `subscribe()` itself, so
 * the topic map exists before any debounce delay.
 */
	...
	describe("integration: concurrent subscribe + connect", () => {
		...
		it("peers discover each other when subscribing and connecting concurrently", async () => {
			...
			await waitForResolved(() => {
				const peer0Topics =
					session.peers[0].services.pubsub.topics.get(TOPIC);
				const peer1Topics =
				...
				expect(
					peer0Topics!.has(
						session.peers[1].services.pubsub.publicKeyHash,
					),
				).to.be.true;
				expect(
					peer1Topics!.has(
						session.peers[0].services.pubsub.publicKeyHash,
					),
				).to.be.true;
			});
		});
