...
describe("pubsub", function () {
			...
			it("line", async () => {
				...
				*/
				...
				for (const [i, peer] of streams.entries()) {
					...
					await waitForResolved(() =>
						expect(peer.stream.getSubscribers(TOPIC)).toHaveLength(1)
					); // all others (except 4 which is not subscribing)
				}
	...
	describe("join/leave", () => {
		...
		const checkSubscriptions = async () => {
			...
			await waitForResolved(() => expect(streams[1].received).toHaveLength(1), {
				timeout: 2000,
				delayInterval: 50
			});
		};
	...
	describe("subscription", () => {
		...
		describe("invalidation", () => {
			it("uses timestamp to ignore old events", async () => {
				...
				expect(streams[1].stream.topics.get(TOPIC_1)!.size).toEqual(0);
				...
				expect(streams[1].stream.topics.get(TOPIC_1)!.size).toEqual(1); // No change, since message was old
