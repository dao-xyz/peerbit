...
Sending a message to 10 peers is easy.

Sending it to 10,000 peers is where things get weird. Not because the payload is large, but because the network starts spending more effort on:
...
It is informed by classic work like [Epidemic Broadcast Trees](https://doi.org/10.1109/SRDS.2007.4365705) (Plumtree-style tree + repair), [HyParView](https://doi.org/10.1109/DSN.2007.56), and modern pubsub practice like [libp2p Gossipsub](https://github.com/libp2p/specs/blob/master/pubsub/gossipsub/README.md), all of which build on older [epidemic/gossip framing](https://doi.org/10.1145/41840.41841).
...
Over the last development cycle we turned that idea into a concrete Peerbit implementation: bounded fanout delivery, shard-aware routing, and capacity-aware join/repair behavior.

This post is a walkthrough of that new implementation. The two interactive sandboxes below are not conceptual mockups; they run the current protocol logic over an in-memory transport so you can inspect delivery and formation behavior directly.

## What we implemented (at a glance)

- **FanoutTree overlay**: bounded broadcast forwarding with join/admission controls and local repair.
- **A split between formation and delivery**: we can evolve join/discovery logic without changing payload forwarding semantics.
- **Shard-based routing**: user topics map to fanout shards (`shard = hash(topic) % shardCount`) for scalable overlay placement.
- **Targeted delivery paths**: explicit-recipient delivery uses DirectStream, while fanout route-token unicast/proxy paths handle economical targeted control inside channels.

## Demo 1: Economic broadcast delivery

For delivery, we implemented bounded-fanout forwarding over shard overlays in Peerbit.
The goal is economic broadcast: each peer forwards to a limited set of children instead of flooding the whole mesh.
If the tree is healthy, one publish is close to **N - 1** payload transmissions, roughly one per subscriber.

This demo is meant to show the implemented delivery behavior directly on a fixed underlay graph (where the bytes actually go).
...
3. Press **Run** to publish messages (by default the flow view includes setup + publish so animation starts right away).
4. Enable **Show flow** to see “comets” moving along edges as data is forwarded.
5. Read the links as: **solid lines = generated underlay graph**, **dashed blue lines = runtime stream connections** that the protocol opens during bootstrap/join/repair. These dashed links are real connections in the simulation and stay visible for a short time after traffic so path changes are easier to inspect.
...
As of February 2026, Peerbit validates this direction with deterministic CI sims and larger local/nightly fanout runs (see the linked spec for presets and thresholds). This post is an architecture + early evidence writeup, not a claim that every target scale is already production-proven.
...
- repair locally so one broken edge does not break delivery ([Epidemic Broadcast Trees](https://doi.org/10.1109/SRDS.2007.4365705)).
...
Delivery is only half the story. We also implemented a formation path so large overlays can be built and repaired without global membership knowledge.
The formation goal is to:
- converge without everyone knowing everyone, and
- respect per-node capacity (so the bootstrap node does not get crushed).

This sandbox visualizes the implemented **FanoutTree join and reattach behavior** over the same in-memory transport: joiners use tracker candidates, attach where capacity exists, and re-form when parents disappear.
...
- **Implemented now**: rooted channel sequencing, bounded fanout forwarding, join/admission controls, local repair, route-token unicast/proxy-publish, and shard-based topic mapping.
...
- **How to read the demos**: Demo 1 focuses on delivery economics; Demo 2 focuses on formation efficiency and reattach behavior under churn.
